...
function TicketCategoryPage() {
  ...
  const [categories, setCategories] = useState(() => getTicketCategories());
  const [events] = useState(() => getEvents());
  const [venues] = useState(() => getVenues());
  ...
  const visibleEventCount = useMemo(() => {
    return new Set(filteredCategories.map((item) => item.eventId)).size;
  }, [filteredCategories]);
  ...
  const refreshCategories = () => setCategories(getTicketCategories());

  const openCreateModal = () => {
    if (!showActions) return;

    if (role === "organizer") {
      const ownEvents = events.filter((eventItem) => eventItem.organizerId === user?.id);
      if (!ownEvents.length) {
  ...
  const handleSubmit = (payload) => {
    ...
    if (formMode === "edit" && editingCategory) {
      ...
      const updated = updateTicketCategory(editingCategory.id, payload);
      if (!updated) {
        toast("Gagal memperbarui kategori tiket.", "error");
        return false;
      }
  ...
  const handleDelete = (category) => {
    ...
    const ok = deleteTicketCategory(category.id);
    if (ok) {
      toast("Kategori dihapus.", "success");
      refreshCategories();
    } else {
      toast("Kategori tiket gagal dihapus.", "error");
    }
